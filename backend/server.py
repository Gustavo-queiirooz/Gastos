...
def register_crud(name: str, coll: str, create_model):
    ...
    async def _update(item_id: str, body: dict):
        ...
        body = {
            ...
            if v is not None and k not in ("id", "_id")
        }
...
async def create_transaction(body: M.TransactionCreate):
    ...
    if (
        ...
        and data["installments"] > 1
        and data.get("type") == "expense"
    ):
        ...
        await db.installment_purchases.insert_one(
            dict(purchase)
        )
        ...
        for i in range(n):
            ...
            c = stamp({
                "description": (
                    f"{data.get('description','Compra')} "
                    f"({i+1}/{n})"
                ),
                "amount": per,
                "due_date": due_date_from_day(
                    base,
                    due_day,
                    i + 1
                ),
    ...
    if (
        data.get("is_split")
        and data.get("split_total")
        and data.get("my_share") is not None
    ):
        ...
        if data.get("person_id") and other > 0:
            ...
            rec = stamp({
                ...
                "description": (
                    f"{data.get('description','Despesa dividida')} "
                    f"(parte de terceiro)"
                ),
                "total": other,
                "received": 0.0,
...
async def delete_recurring(
    ...
    await db.recurring.delete_one({
        "id": item_id
    })

    await db.commitments.delete_many({
        "origin_id": item_id,
        "status": {
            "$in": [
                "a_vencer",
...
async def list_commitments(
    ...
    return await db.commitments.find(
        q,
        {"_id": 0}
    ).sort(
        "due_date",
...
async def update_commitment(
    ...
    return await db.commitments.find_one(
        {"id": item_id},
        {"_id": 0}
    )
...
async def delete_commitment(
    ...
    await db.commitments.delete_one({
        "id": item_id
    })
...
async def pay_commitment(
    item_id: str,
    account_id: Optional[str] = None
):

    c = await db.commitments.find_one(
        {"id": item_id},
        {"_id": 0}
    )
...
async def create_installment(
    body: M.InstallmentPurchaseCreate
):
...
async def list_installments():
    ...
    return await E.fetch_all(
        "installment_purchases"
    )


# =========================================================
# EXCLUIR UM PARCELAMENTO INTEIRO
# =========================================================

@api.delete("/installment-purchases/{item_id}")
async def delete_installment_purchase(
    item_id: str
):

    # Verifica se o parcelamento existe
    purchase = await db.installment_purchases.find_one(
        {"id": item_id},
        {"_id": 0}
    )
    ...
    # Busca todas as parcelas vinculadas
    installments = await db.commitments.find(
        {
            "origin": "installment",
            "origin_id": item_id
        },
    ...
    # ela gerou uma transação.
    # Excluímos também essas transações
    # para não deixar um gasto "órfão".
    if installment_ids:
    ...
    # a vencer, atrasadas e pagas.
    await db.commitments.delete_many({
        "origin": "installment",
        "origin_id": item_id
    })

    # Finalmente exclui o parcelamento principal.
    await db.installment_purchases.delete_one({
        "id": item_id
    })

    return {
        "ok": True,
        "deleted_purchase": item_id,
        "deleted_installments": len(
            installment_ids
        )
    }
...
async def delete_receivable(
    ...
    await db.receivables.delete_one({
        "id": item_id
    })
...
async def receive_receivable(
    ...
    return await db.receivables.find_one(
        {"id": item_id},
        {"_id": 0}
    )
...
async def create_loan(
    ...
    if data["kind"] == "borrowed":
        ...
        for i in range(paid, n):
            ...
            c = stamp({
                "description": (
                    f"{data.get('institution','Empréstimo')} "
                    f"({i+1}/{n})"
                ),
                "amount": data.get(
                    "installment_value",
...
async def update_need(
    ...
    return await db.needs.find_one(
        {"id": item_id},
        {"_id": 0}
    )
...
async def contribute_goal(
    item_id: str,
    body: M.GoalContribute
):

    g = await db.goals.find_one(
        {"id": item_id},
        {"_id": 0}
    )
...
async def delete_challenge(
    ...
    await db.challenges.delete_one({
        "id": item_id
    })
...
async def planning(
    ...
    return await E.compute_planning(
        months
    )
...
async def posso_comprar(
    body: M.PossoComprarCreate
):

    return await E.compute_posso_comprar(
        body.amount,
        body.installments
    )
...
async def seed_defaults():
    ...
    if await db.challenges.count_documents({}) == 0:
        ...
        ch = stamp({
            "name": "Desafio dos 365 dias",
            "mode": "crescente",
            "start_date": M.today_iso()
        })
